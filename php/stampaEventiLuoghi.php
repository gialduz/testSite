    ...
    function stampaLuoghiAmministratore($conn) {
        // QUERY SELECT TABELLA EVENTO
    
    $sql = "SELECT * FROM Luogo AS L ORDER BY id";
    $result = $conn->query($sql);   
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            $daRitornare.= "<tr class='nr'>"
                ."<td>" . $row["id"]."</td><td>" . $row["lettera"]. "</td><td style='background-color: ".$row["colore"]."'>" 
                . $row["colore"]. "</td><td class='uppato'>" 
                . $row["nome"].  "</td><td>" 
                . $row["latitudine"]."</td><td>" 
                . $row["longitudine"]. "</td><td>" 
                . $row["citta"]. "</td><td>" 
                . $row["tipo_via"]. "</td><td>"
                . $row["via"]. "</td><td>"
                . $row["numero_civico"]. "</td>"."<td><a href='#'><img src='../img/edit_icon.png' style='max-width:25px' class='resp editBtn'></a></td>" 
                ."<td><a href='#'><img src='../img/cancel_icon.png' style='max-width:25px' class='resp delBtn'></a></td>" 
                ."</tr>";
        }
    } else {
